          ...
          <div class="p-6 bg-white border-b border-gray-200">
            <form @submit.prevent="handleSubmit">
              ...
              <div class="flex items-center justify-end mt-4">
                <router-link
                  :to="{ name: 'posts.show', params: { id: postId }}"
...
onMounted(async () => {
  ...
  if (!postsStore.canEditPost(postsStore.currentPost)) {
    router.push({ name: 'posts.show', params: { id: postId }})
    return
  }
  form.title = postsStore.currentPost.title
  form.content = postsStore.currentPost.content
  form.tags = postsStore.currentPost.tags?.join(', ') || ''
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await postsStore.updatePost(postId, postData)
    router.push({ name: 'posts.show', params: { id: postId }})
  } catch (error) {
